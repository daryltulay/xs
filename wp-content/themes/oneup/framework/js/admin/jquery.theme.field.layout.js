	...
	function PeFieldLayout(target, conf) {
		...
		function addBlock(type,where,fun,animate) {
			...
			if (isNew && module.container) {
				...
				var msg = 'Use <i class="pe_icon_plus"></i> below to add more blocks to this %0'.format(module.conf.messages.type);
				try {
				...
				body.find("> ul.pe_block_container:first")
					.append('<li class="pe_help">%0</li>'.format(msg));
			}
